...
function App() {
  const [data, setData] = useState(
    vocabularyData.map(set => ({ ...set, words: [...set.words] }))
  );
  ...
  // IMPORTANT: Load voices for iPhone / Safari
  useEffect(() => {
    const loadVoices = () => {
    ...
    <div style={styles.container}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
        <button
          style={styles.button}
          onClick={() =>
            setIsEnglishToKorean(prev => !prev)
          }
        >
          🔁 Switch to{' '}
          {isEnglishToKorean
